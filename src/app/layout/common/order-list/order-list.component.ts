...
export class OrderListComponent implements OnInit, OnDestroy {
    ...
    this.channelname=this.order.channel_name
    ...
    invoice:any
    donloadInvoice(order){
        let payload={
            'order_id':order.order_id,
            // 'payment_term':this.documentForm.get('payment_term').value,
            'plateform_type':order.plateform_type
        }
        this._orderService.order_invoice_download(payload).subscribe(res=>{
            console.log("res",res);
            this.invoice =res.payload

        //     console.log("invoice",this.invoice.invoice);
        // window.open(this.invoice)
        })
    }
    ...
    openQuickChat(channel_id) {
        ...
        // openQuickChat(user_id) {
            ...
            // this._quickChatService.sendChannelId(channel_id)
            if(this.userDetails.userType==="brand"){
                let payload={
                    "user_id":channel_id.influ_id,
            ...
            this.order.messageCount=0
            console.log("this.order.messageCount",this.order.messageCount);
            
            }else{
                let payload={
                    "user_id":channel_id.brand_id,
                    "channel_id":channel_id.channelid,
                    // "type":"order",
                    "plateform_type":channel_id.plateform_type
                }
