...
@Component({
  selector: 'insta-channel-list',
  templateUrl: './insta-channel-list.component.html',
  styleUrls: ['./insta-channel-list.component.scss']
})
export class InstaChannelListComponent implements OnInit {
  @Input() channel: any;
...
this.dontShowMe= JSON.parse(localStorage.getItem("userInfo"))
      ...
      if (this.userData.plan_id === 1 && channel.EstViews > 50000) {
          ...
          this.publicForm = this._formBuilder.group({
              title: 'Upgrade Plan',
              message: ' 50K+ Channel Reveal, is not for Free plan , it is only for bussiness plan.',
              icon: this._formBuilder.group({
                  show: true,
                  name: 'heroicons_outline:exclamation',
                  color: 'warn'
  ...
  onHireClick(channel) {
      ...
      let test=this._campaignService.setHirechannelData(channel)
  ...
  ProposalClick(){
    console.log("this.response.plateform_type 2",this.response.plateform_type);
...
console.log("result",result);
  ...
  markFavourite(id) {
      ...
      this._channelService.likeChannel({ channel_id: id,plateform_type:2 }).subscribe(data => {
  ...
  markUnFavourite(id) {
      this._channelService.dislikeChannel({ channel_id: id,plateform_type:2 }).subscribe(data => {
...
console.log("alphaNumeric : " + alphaNumeric );
  ...
  openQuickChat(user_id) {
      ...
      if(this.channel.is_revealed){
        // if(user_id.name==='Instagram'){
            this._quickChatService.sendChannelId(user_id)

            let payload = {
                "channel_id":user_id.channelid,
