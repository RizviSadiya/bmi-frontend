...
import { CampaignsAlertPopupComponent } from 'app/modules/admin/apps/campaigns/campaigns-alert-popup/campaigns-alert-popup.component';
import { RevealChannelpopupComponent } from 'app/modules/admin/apps/campaigns/reveal-channelpopup/reveal-channelpopup.component';
// import WhatsAppIcon from '@mui/icons-material/WhatsApp';

@Component({
    selector: 'influencer-channel',
    templateUrl: './influencer-channel.component.html',
    encapsulation: ViewEncapsulation.None
})
export class InfluencerChannelComponent implements OnInit, OnDestroy {
    @Input() channel: any;
    @Input() creditCost: any;
...
console.log("dontShowMe",this.dontShowMe);
    ...
    showSimilar(channel){
        ...
        this._campaignService.showSimilar(payload).subscribe(res=>{
            if(res.success){
                ...
                this._campaignService.setPlateForm("youtube")
    ...
    un_revealchannel(id, credit_cost,channel,el:HTMLElement){
        if(this.channel.is_revealed !=true){
            this.publicForm = this._formBuilder.group({
                ...
                message: 'To access this feature, please reveal the channel first.',
                // message: 'Only paid users can use this feature.',
                icon: this._formBuilder.group({
                    show: true,
                    name: 'heroicons_outline:exclamation',
    ...
    un_revealSimilar(id, credit_cost,channel,el:HTMLElement){
        if(this.channel.is_revealed !=true){
            this.publicForm = this._formBuilder.group({
                title: 'Reveal Request',
                message: 'To access this feature, please reveal the channel first.',
            ...
            dialogRef.afterClosed().subscribe((result) => {
                if (result === "confirmed") {
                    ...
                    let obj={
                        ...
                        "plateform_type":this.channel.plateform_type
                    }
                  this._campaignService.sendWhatsappRequest(obj).subscribe(res=>{
                    if(res.success){
                        this.channel.is_whatsup_send = true;
                        this._changeDetectorRef.markForCheck();
    ...
    revealChannel(id, credit_cost,channel,el:HTMLElement) {
        console.log("channel",channel.EstViews);
this.dontShowMe= JSON.parse(localStorage.getItem("userInfo"))
        if(this.creditCost < credit_cost){
            this.askForTopupOrUpgrade()
        }
        if (this.userData.plan_id === 1 && channel.EstViews > 50000) {

            this.publicForm = this._formBuilder.group({
                title: 'Premium Influencer spotted!',
                message: ' Premium influencers are only accessible to <b> Business plan <b> users.',
                icon: this._formBuilder.group({
                    show: true,
                    name: 'heroicons_outline:exclamation',
                    color: 'warn'
    ...
    ProposalClick(){
        ...
        let payload={
            ...
            "plateform_type":this.response.plateform_type
        }
...
console.log("result",result);
    ...
    markFavourite(id) {
        ...
        this._channelService.likeChannel({ channel_id: id ,plateform_type:1}).subscribe(data => {
    ...
    markUnFavourite(id) {
        this._channelService.dislikeChannel({ channel_id: id,plateform_type:1 }).subscribe(data => {
...
// console.log("alphaNumeric : " + alphaNumeric );
    ...
    openQuickChat(channel) {
        console.log("openQuickChat", channel);
        if(this.channel.is_revealed){
            this._quickChatService.sendChannelId(channel)
            let payload={
                "user_id":channel.influ_id,
